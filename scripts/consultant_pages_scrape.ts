...
const DEBUG = true;
const CHROME_EXECUTABLE_PATH = 'C:/Program Files (x86)/Google/Chrome/Application/chrome.exe';
const PAGE_TIMEOUT = 60000; // Max time per page (ms)
const ELEMENT_TIMEOUT = 10000; // Max time to wait for an element (ms)

// --- Type Definition ---
// (Keep the Consultant interface as before)
...
function parsePrice(priceString: string | null | undefined): { price: number | null, currency: string | null } {
    if (!priceString) {
...
async function processUrl(browser: Browser, profileUrlBase: string): Promise<ProcessResult> {
    let page: Page | null = null;
    const profileUrlAr = profileUrlBase.replace(BASE_URL, AR_DETAIL_PAGE_BASE_URL);
    ...
    try {
        ...
        await page.goto(profileUrlAr, { waitUntil: 'domcontentloaded', timeout: PAGE_TIMEOUT }); // Faster wait condition
        log(`[Task Navigated] ${profileUrlAr}`);

        // --- Scrape data points ---
        const results = await Promise.allSettled([
            // Name (0)
            page.waitForSelector('.profile_left .consultant_text h3', { timeout: ELEMENT_TIMEOUT })
                .then(() => page?.$eval('.profile_left .consultant_text h3', h3 => h3.textContent?.trim() || null)),
            // Image URL (1)
            page.waitForSelector('.profile_left .consultant_img img', { timeout: ELEMENT_TIMEOUT })
                .then(() => page?.$eval('.profile_left .consultant_img img', img => img.getAttribute('src') || null)),
            // Fields (2) - Allow this to fail more easily
            page.waitForSelector('.profile_left .consultant_field .field', { timeout: ELEMENT_TIMEOUT / 2 }) // Shorter timeout for optional fields
                .then(() => page?.$$eval('.profile_left .consultant_field .field', spans =>
                    spans.map(span => span?.textContent?.trim() || '').filter(Boolean)))
                .catch(() => []),
            // Description (3)
            page.waitForSelector('div.about_consultant ul li', { timeout: ELEMENT_TIMEOUT })
                .then(() => page?.$eval('div.about_consultant ul li', li => li.textContent?.trim() || null)),
            // Price String (4)
            page.waitForSelector('.profile_left .contact_consultant h3', { timeout: ELEMENT_TIMEOUT })
                .then(() => page?.$eval('.profile_left .contact_consultant h3', h3 => h3.textContent?.trim() || null))
        ]);

        const name = results[0].status === 'fulfilled' ? results[0].value : null;
        let imageUrl = results[1].status === 'fulfilled' ? results[1].value : null;
        const fields: string[] = results[2].status === 'fulfilled' && Array.isArray(results[2].value) ? results[2].value : []; // Ensure it's always a string array
        const description = results[3].status === 'fulfilled' ? results[3].value : null;
        const priceString = results[4].status === 'fulfilled' ? results[4].value : null;
        ...
        log(`[Task Error] Failed processing ${profileUrlAr}:`, error instanceof Error ? error.message : error);
...
async function runScraper(): Promise<ProcessResult[]> {
    ...
    log(`Starting parallel scraping for ${consultantProfileUrls.length} URLs with concurrency ${MAX_CONCURRENCY}...`);
    ...
    try {
        ...
        const tasks = pooledMap(
            MAX_CONCURRENCY,
            consultantProfileUrls,
            (url) => processUrl(browser!, url)
        );

        // Collect results as they finish
        for await (const result of tasks) {
            if (result) { // Only add successful results (non-null)
                results.push(result);
            }
            // Optional: Log progress periodically
            if (results.length % 10 === 0) {
                 log(`Progress: ${results.length}/${consultantProfileUrls.length} URLs processed.`);
            }
        }

    } catch (error) {
        console.error('An critical error occurred during the scraping orchestration:', error);
    } finally {
        if (browser) {
            log('Closing browser...');
            await browser.close();
    ...
    log(`Scraping finished. Successfully extracted data for ${results.length}/${consultantProfileUrls.length} consultants.`);
    return results;
}
...
if (import.meta.main) {
    runScraper()
        .then(async (data) => {
            const successfulData = data.filter((item): item is Consultant => item !== null); // Type guard
